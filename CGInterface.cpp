...
bool ShaderOneInterface::PerSessionInit(CGInterface *cgi) {
	...
	vertexRadius = cgGetNamedParameter(vertexProgram, "radius");
	vertexCenter = cgGetNamedParameter(vertexProgram, "center");
	vertexFraction = cgGetNamedParameter(vertexProgram, "fraction");
...
void ShaderOneInterface::PerFrameInit() {
	...
	cgGLSetParameter1f(vertexRadius, scene->animationRadius);
	cgGLSetParameter1f(vertexFraction, scene->animationFraction);
	cgGLSetParameter3f(vertexCenter, scene->animationCenter[0],
		scene->animationCenter[1], scene->animationCenter[2]);

	static float blueHue = 0.5f;
	cgGLSetParameter1f(fragmentBlueHue, blueHue);
	cgGLSetParameter3f(fragmentLight, scene->light[0], scene->light[1],
		scene->light[2]);
